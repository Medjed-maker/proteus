...
class TestSearch:
    ...
    @staticmethod
    def _raise_should_not_rebuild_ipa_index(
        lexicon_lookup: object,
    ) -> search_module.IpaIndex:
        raise AssertionError("should not rebuild IPA index")

    @staticmethod
    def _raise_should_not_call_to_ipa(
        query: str, dialect: str = "attic"
    ) -> NoReturn:
    ...
    def _fake_score_stage(
        ...
        return [
            SearchResult(
                lemma=TestSearch._extract_headword(lexicon_map[entry_id])
                if use_headword
                else entry_id,
                confidence=1.0,
    ...
    def test_token_proximity_fallback_does_not_drop_same_length_exact_match_beyond_previous_limit(
        ...
        monkeypatch.setattr(
            search_module, "to_ipa", lambda query, dialect="attic": "aːi"
        )
        ...
        lexicon.append(
            {"id": "ZZZ", "headword": "target", "ipa": "aːi", "dialect": "attic"}
        )
    ...
    def test_token_proximity_fallback_uses_cached_token_counts(
        ...
        monkeypatch.setattr(
            search_module, "to_ipa", self._raise_should_not_call_to_ipa
        )
        monkeypatch.setattr(
        ...
        search(
            ...
            unigram_index={},
            language="test",
            converter=lambda query, dialect: "query-ipa",
        )

        assert token_calls.count("aː") == 1
        assert token_calls.count("i") == 1
        assert token_calls.count("ou") == 1
    ...
    def test_search_uses_prebuilt_lexicon_map_without_retokenizing_entries(
        ...
        assert "aː" not in token_calls
        assert "i" not in token_calls
        assert "ou" not in token_calls
    ...
    def test_prebuilt_lexicon_map_reuses_token_fallback_lookup_without_rebuilding(
        ...
        def fake_annotate_search_results(
            ...
            **_kwargs: Any,
            ...
            captured["annotate_lookup_is_prebuilt"] = (
                lexicon_map is prebuilt_lexicon_map
            )
            return results
        ...
        monkeypatch.setattr(search_module, "_score_stage", fake_score_stage)
        monkeypatch.setattr(
            search_module, "_annotate_search_results", fake_annotate_search_results
        )
    ...
    def test_search_does_not_tokenize_full_lexicon_when_k2_seed_hits(
        ...
        def fail_build_lexicon_map(
            lexicon: object, **_kwargs: object
        ) -> search_module.LexiconMap:
    ...
    def test_seeded_fullform_adds_length_proximate_candidates_without_exact_match(
        ...
        prebuilt_lexicon_map = {
            entry["id"]: LexiconRecord(
                ...
                ipa_tokens=("x",)
                * (8 if index < search_module._MIN_STAGE2_CANDIDATES else 3),
            )
    ...
    def test_partial_query_token_proximity_fallback_is_bounded_by_default(
        ...
        monkeypatch.setattr(
            search_module, "_match_partial_query", tracking_match_partial_query
        )
        monkeypatch.setattr(
            partial_module, "_match_partial_query", tracking_match_partial_query
        )
        monkeypatch.setattr(
            search_module,
            "_annotate_search_results",
        ...
        lexicon.append(
            {"id": "TARGET", "headword": "target", "ipa": "x a", "dialect": "attic"}
        )
        lexicon.extend(
            {
                "id": f"T{index:04d}",
        ...
        assert (
            captured["fallback_max_candidates"]
        ...
        assert len(captured["candidate_ids"]) == search_module._partial_candidate_limit(
            1
        )
        ...
        def fake_build_lexicon_map(
            ...
            **_kwargs: Any,
            ...
            return {
                ...
                "L2": LexiconRecord(
                    entry=lexicon[1], token_count=2, ipa_tokens=("p", "a")
                ),
            }
    ...
    def test_search_debug_logs_redacted_query_label_for_performance_metrics(
        ...
        lexicon = [
            {"id": "L1", "headword": "alpha", "ipa": "p t e n", "dialect": "attic"}
        ]
    ...
    def test_search_skips_building_unigram_index_for_pure_vowel_query(
        ...
        def fake_build_kmer_index(lexicon: object, k: int = 2) -> dict[str, list[str]]:
    ...
    def test_search_accepts_explicit_dialect(
        self, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        captured: dict[str, object] = {}
    ...
    def test_search_reuses_prebuilt_index(
        self, monkeypatch: pytest.MonkeyPatch
    ) -> None:
    ...
    def test_public_prebuilt_lexicon_map_preserves_multicharacter_exact_match(
        self,
    ) -> None:
        """Public build_lexicon_map defaults should stay compatible with default search."""
        lexicon = [
            {
                "id": "L1",
                "headword": "target",
        ...
        lexicon_map = search_module.build_lexicon_map(lexicon)
        ...
        assert not any(
            application.rule_id.startswith(search_module.OBSERVED_PREFIX)
            for application in results[0].rule_applications
        )

    def test_search_with_empty_prebuilt_ipa_index(
        self, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        ...
        results = search(
            "πτην", lexicon, matrix={}, max_results=1, prebuilt_ipa_index={}
        )
    ...
    def test_koine_seed_index_keeps_attic_target_within_stage2_limit(
        ...
        """Koine seed index should keep attic target within stage2 limit when seeded from koine."""
        monkeypatch.setattr(
            search_module,
            "to_ipa",
        ...
        lexicon = [
            {
                "id": f"D{index:03d}",
                "headword": f"distractor-{index:03d}",
                "ipa": "nnals",
                "dialect": "attic",
            }
        ...
        results = search(
            ...
            unigram_index=build_kmer_index(lexicon, k=1),
        )
    ...
    def test_normalized_default_language_keeps_koine_seed_defaults(
        self, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        """Normalized Ancient Greek language IDs should still fill public defaults."""
        captured: dict[str, list[str]] = {}

        def fail_fallback(*_args: object, **_kwargs: object) -> NoReturn:
            raise AssertionError("normalized default language should seed without fallback")
    ...
    def test_non_default_language_internal_seed_uses_literal_fallback(
        ...
        """Internal seed path must not reapply Ancient Greek defaults."""
        lexicon = (
            {
                "id": "TARGET",
                "headword": "target",
        ...
        def fail_fallback(*_args: object, **_kwargs: object) -> NoReturn:
            raise AssertionError("non-default language should seed without fallback")
            ...
            **_kwargs: object,
            ...
            return [
                SearchResult(
                    lemma="target",
                    confidence=1.0,
                    dialect_attribution="lemma dialect: test",
                    entry_id="TARGET",
                )
            ]
