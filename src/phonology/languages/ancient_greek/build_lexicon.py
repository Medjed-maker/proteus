...
_FINGERPRINT_INPUTS = (
    ...
    Path("src/phonology/languages/ancient_greek/lsj_extractor.py"),
    Path("src/phonology/languages/ancient_greek/transliterate.py"),
    Path("src/phonology/languages/ancient_greek/betacode.py"),
...
def _compute_file_hash(path: Path) -> str:
    """Return the SHA-256 hex digest of a file using chunked reads."""
    digest = hashlib.sha256()
    with path.open("rb") as fh:
        while chunk := fh.read(_FILE_HASH_CHUNK_SIZE):
            digest.update(chunk)
...
def _validate_lsj_repo_dir_target(lsj_repo_dir: Path, project_root: Path) -> None:
    ...
    _validate_lsj_repo_dir_parent_chain(lsj_repo_dir, project_root)
...
def _validate_lsj_repo_dir_parent_chain(
    lsj_repo_dir: Path, project_root: Path
) -> None:
    """Validate parent directories before creating an LSJ checkout target."""
    resolved_project_root = project_root.resolve(strict=False)
    resolved_target = lsj_repo_dir.resolve(strict=False)
    if not resolved_target.is_relative_to(resolved_project_root):
        logger.error(
            "Refusing to create lsj_repo_dir %s: resolved path %s is not under project root %s",
            lsj_repo_dir,
            resolved_target,
            resolved_project_root,
        )
        raise RuntimeError(
            "Safety check failed: lsj_repo_dir "
            f"{resolved_target} is not under project root {resolved_project_root}"
        )

    for parent in lsj_repo_dir.parents:
        if parent.is_symlink():
            logger.error(
                "Refusing to create lsj_repo_dir %s: parent directory %s is a symbolic link",
                lsj_repo_dir,
                parent,
            )
            raise RuntimeError(
                "Safety check failed: parent directory "
                f"{parent} for lsj_repo_dir {lsj_repo_dir} is a symbolic link; "
                "remove the symlink manually before retrying."
            )

        resolved_parent = parent.resolve(strict=False)
        if not resolved_parent.is_relative_to(resolved_project_root):
            logger.error(
                "Refusing to create lsj_repo_dir %s: parent directory %s resolves to %s outside project root %s",
                lsj_repo_dir,
                parent,
                resolved_parent,
                resolved_project_root,
            ...
            raise RuntimeError(
                "Safety check failed: parent directory "
                f"{resolved_parent} for lsj_repo_dir {lsj_repo_dir} is not under "
                f"project root {resolved_project_root}"
            )
        if resolved_parent == resolved_project_root:
            break
...
def _run_extractor(
    *,
    ...
    resolved_supplemental_path: Path | None
    if supplemental_path is not None:
        resolved_supplemental_path = supplemental_path
    else:
        supplemental_candidate = _default_supplemental_path(_default_project_root())
...
def _tracked_input_records(project_root: Path, xml_dir: Path) -> list[dict[str, Any]]:
    ...
    from phonology.languages.ancient_greek.lsj_extractor import find_xml_files
...
def _is_reusable_output_document(*, project_root: Path, output_path: Path) -> bool:
    ...
    Returns:
        True if the output file exists, contains valid JSON, is a JSON object,
            and passes schema validation via the Ancient Greek LSJ extractor.
    ...
    try:
        ...
        from phonology.languages.ancient_greek.lsj_extractor import validate_document
