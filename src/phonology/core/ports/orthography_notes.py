...
# Language-independent base note kinds. Language plugins may extend this
# vocabulary with their own kinds (the payload's ``kind`` field is widened to
# ``str`` so plugins can supply additional, language-specific values).
...
class OrthographicNotePayload:
    ...
    # ``str`` rather than ``OrthographicNoteKind`` so language plugins may
    # supply their own kinds beyond the language-independent base set.
    kind: str
    label: str
    messages: tuple[str, ...]
