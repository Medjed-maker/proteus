...
from ._rule_tokenize import (
    _resolve_always_match_contexts,
    _resolve_phone_inventory,
)
...
def _resolve_vowel_phones(
    vowel_phones: frozenset[str] | Sequence[str] | None = None,
) -> frozenset[str]:
    """Return explicit vowels or the current default profile's vowel inventory."""
    if vowel_phones is not None:
        return frozenset(vowel_phones)

    from ..core.ports.profiles import get_default_language_profile

    return frozenset(get_default_language_profile().vowel_phones)
...
def _is_vowel(
    ...
    return token in _resolve_vowel_phones(vowel_phones)
...
def _is_consonant(
    ...
    return token is not None and token not in _resolve_vowel_phones(vowel_phones)
    ...
    *,
    ...
    if tail_tokens is None:
        tail_tokens = tuple(
            tokenize_ipa(
                match.group(1),
                phone_inventory=_resolve_phone_inventory(phone_inventory),
            )
        )
    if not tail_tokens:
        return False
...
def _matches_word_boundary(
    ...
    del lemma_start, query_start, context_tail_tokens, vowel_phones
    next_token = _lookup_next_token(
        lemma_tokens,
        query_tokens,
        lemma_end=lemma_end,
...
def _matches_word_initial_before_vowel(
    ...
    return _is_vowel(next_token, vowel_phones)
...
def _matches_intervocalic(
    ...
    return _is_vowel(prev_token, vowel_phones) and _is_vowel(
        next_token,
        vowel_phones,
    )
...
def _matches_nasal_consonant(
    ...
    return next_token in _NASAL_PHONES and _is_consonant(
        following_token,
        vowel_phones,
    )
...
def _matches_after_eir(
    ...
    del lemma_end, query_end, context_tail_tokens, vowel_phones
    prev_token = _lookup_prev_token(
        lemma_tokens,
        query_tokens,
        lemma_start=lemma_start,
...
def _matches_context(
    ...
    *,
    ...
    if normalized in _resolve_always_match_contexts(always_match_contexts):
        return True
    resolved_vowel_phones = _resolve_vowel_phones(vowel_phones)
