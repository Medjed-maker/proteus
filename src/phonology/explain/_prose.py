...
from ._types import RuleApplication
...
class Explanation:
    ...
    target_ipa: str
    distance: float
    steps: Sequence[RuleApplication]
    prose: str = ""

    def __post_init__(self) -> None:
        """Store steps immutably so callers cannot mutate explanation state."""
...
def to_prose(explanation: Explanation) -> str:
    ...
    if explanation.steps:
        step_summary = "; ".join(
            (
                f"{step.rule_name} ({step.input_phoneme} -> {step.output_phoneme} "
