...
from ._rule_paths import _resolve_and_validate_rules_dir
...
def load_rules(rules_dir: Path | str) -> dict[str, dict[str, Any]]:
    ...
    Args:
        rules_dir: Path to the rules directory. Relative inputs are resolved
            from the packaged rules base, so both ``"<language_id>"`` and
            ``"data/rules/<language_id>"`` resolve to the same runtime asset.
...
def _extract_rule_file_version(
    ...
    # We attempt to use yaml.compose to obtain a low-level AST (composed).
    # This allows us to inspect raw yaml.ScalarNode objects and their tags,
    # enabling us to distinguish between raw representation types (e.g. distinguishing
    # between float tags and decimal representations) before they get coerced.
    # If yaml.compose fails (raising YAMLError) or does not yield a MappingNode,
    # composed remains None (or non-MappingNode).
    try:
        composed = yaml.compose(content)
    except yaml.YAMLError:
        composed = None

    # If composed is a MappingNode, we extract the version ScalarNode and process it via
    # _extract_scalar_node_value. If composed is None (due to parse exception) or not a MappingNode,
    # we intentionally fall back to the standard safe_load-derived dict (document)
    # and retrieve the version string/value using document.get(), which is validated
    # by _is_valid_rule_version_value.
