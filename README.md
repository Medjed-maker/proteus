...
In the UI, the orthographic-note group separates three distinct relations
between the query and the candidate:

- `Current candidate: παιδίο → παιδίον` is the search candidate chosen by ranking.
- `Writing-system alternative reading: παιδίο → παιδίου (paidiou)` is a
  writing-system reading aid.
- `Pre-403/2 BCE Attic inscriptional spelling: παιδίο (paidiō)` shows the
  inscriptional spelling and reading.

The normalized form `παιδίου (paidiou)` is the later or standard-form reading.
