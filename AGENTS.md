...
Recent history follows Conventional Commit style such as `feat: implement secure phonological distance matrix loading`. Continue using prefixes like `feat:`, `fix:`, and `test:` with concise summaries. Pull requests should describe the behavior change, list affected modules or data files, and note test coverage added or updated. Include screenshots only when changing `src/proteus/web/index.html` or other user-visible output.
