...
- [x] `src/api/_models.py` に `ResponseMeta(BaseModel)` を追加。フィールド:
  - [x] `api_version: str` — REST API 契約バージョン (例: `"1.0"`)。
  - [x] `schema_version: str` — `SearchResponse` JSON Schema の semantic version (例: `"1.0.0"`)。
  - [x] `engine_version: str` — `proteus` package version (`_load_app_version()` を再利用)。
  - [x] `data_versions: DataVersions` — 既存 `DataVersions` をネスト。
  - [x] `ruleset_versions: dict[str, str]` — 言語別 rules version (例: `{"ancient_greek": "0.1.0"}`)。`get_rules_version()` を呼ぶ。
  - [x] `request_id: str` — UUID4 hex string (length 32)。
  - [x] `timestamp: str` — ISO 8601 UTC string (`datetime.now(timezone.utc).isoformat()`)。
  - [x] `verification_url: str` — deterministic URL (確定済み設計判断参照)。
  - [x] `request_echo: RequestEcho | None` — sanitized request echo (1.2 参照)。`/languages` `/version` では `None` 可。
- [x] `ResponseMeta` は `VersionInfo` と別モデルとして設計する。`ResponseMeta` は `VersionInfo` と共通の version fields を共有し、request metadata / data metadata を追加で提供する。`VersionInfo` の既存 field 名・型・意味は変更せず、`LanguagesResponse.meta` の移行後も client が未知 field を無視すれば継続動作できるようにする。
- [x] `ResponseMeta` の `timestamp` field validator は `DataVersions._validate_timestamp` と同じパターンで検証する。これは `VersionInfo` の validator ではなく、関連する別モデルである `ResponseMeta` 側の追加 metadata 検証として実装する。
- [x] `ResponseMeta` を `__all__` に追加。

#### 1.2 新規 Pydantic モデル: `RequestEcho`
- [x] `src/api/_models.py` に `RequestEcho(BaseModel)` を追加。フィールド:
  - [x] `query_form: str`
  - [x] `language: str`
  - [x] `dialect_hint: str`
  - [x] `max_candidates: int`
  - [x] `response_language: Literal["en", "ja"]`
- [x] `RequestEcho` は `model_config = ConfigDict(frozen=True)`。
- [x] `orthography_hint` (deprecated) は echo に含めない。
- [x] `RequestEcho` を `__all__` に追加。

#### 1.3 `SearchResponse` への組み込み
- [x] `SearchResponse` (`src/api/_models.py:379-405`) に `meta: ResponseMeta` を追加。default factory なし — `main.py` が必ず構築。
- [x] `data_versions` トップレベルは互換のため残す。docstring に `meta.data_versions` も同値であることを明記。
- [x] `model_config = ConfigDict(extra="ignore")` を `SearchResponse` に明示し、unknown fields の追加を非破壊扱いとする規約を schema に固定。

#### 1.4 Request ID & verification URL 生成 helper
- [x] `src/api/_request_context.py` (新規) を作成。
  - [x] `generate_request_id() -> str` — `uuid.uuid4().hex` を返す。
  - [x] `resolve_public_base_url(fastapi_request: Request) -> str` — env `PROTEUS_PUBLIC_BASE_URL` 優先、未設定なら `str(fastapi_request.base_url)`。
  - [x] `build_verification_url(base: str, request: SearchRequest) -> str` — `urllib.parse.urlencode` で組み立て。クエリパラメータ: `q`, `language`, `dialect`, `max_candidates`, `response_language`。
  - [x] `build_request_echo(request: SearchRequest) -> RequestEcho` — sanitized echo を組み立て。
- [x] `src/api/_response_meta.py` (新規) を作成:
  - [x] `build_response_meta(*, request_id: str, request: SearchRequest | None, base_url: str | None, engine_version: str, data_versions: DataVersions, ruleset_versions: dict[str, str]) -> ResponseMeta` — REST/MCP 両方から呼ぶ共通ヘルパー。
  - [x] このモジュールは `api._constants`, `api._models`, `api._request_context` のみ import し、`api.main` を import しない (circular import 回避)。

#### 1.5 Request-ID middleware
- [x] `src/api/main.py` に `@app.middleware("http")` の `_add_request_id` を追加。
  - [x] 受信 header `X-Request-ID` を尊重 (validation: hex, length 8–64)。
  - [x] 無い/不正なら `uuid.uuid4().hex` で生成。
  - [x] `request.state.request_id = ...` に格納。
  - [x] response header に `X-Request-ID` を必ず付ける。
- [x] middleware 順序: security headers の前に置く (エラー時にも `X-Request-ID` が付くように)。
- [x] `search()` 内の request_id はこの middleware の結果を取り出す: `request_id = fastapi_request.state.request_id`。

#### 1.6 Engine / ruleset version 解決
- [x] `_build_ruleset_versions(language: str) -> dict[str, str]` を `src/api/main.py` に追加。
  - [x] 入力 language の `profile.rules_dir` から `get_rules_version()` を呼ぶ (`src/phonology/explainer.py`)。
  - [x] 結果は `{language_id: "<max_version>"}` 形式。
  - [x] 例外時は空 dict (warn ログ)。
- [x] `_APP_VERSION` (`src/api/main.py:111`) を `engine_version` として `ResponseMeta` に流す。

#### 1.7 `search()` の組み立てを書き換え
- [x] `SearchResponse` 生成時に `meta=build_response_meta(...)` を必ず渡すよう修正 (`src/api/main.py:632-650`)。
- [x] `meta.request_echo` には sanitized request を入れる。
- [x] PII リスクは古典語クエリでは低いが、`PROTEUS_LOG_RAW_SEARCH_QUERY` が false の場合 echo は出すがログには出ないことをコメントで明記。
...
- [x] 章 1 実装時は、章 3 で確立済みの `API_VERSION` / `SCHEMA_VERSION` constants を参照する。`ResponseMeta.api_version` は `src/api/_constants.py` から `API_VERSION` を import し、schema/version fields が必要なら `SCHEMA_VERSION` も同じ path から import する。`VersionInfo.api_version` と同じ import path を使い、runtime で `ResponseMeta.api_version` と `VersionInfo.api_version` が同一定数を参照している状態を維持する。

#### 1.9 OpenAPI export
- [x] `scripts/export_openapi.py` (新規) を作成。
  - [x] `from api.main import app` してから `app.openapi()` を取り、`json.dump` で `docs/api/openapi.json` に書き出す。
  - [x] CLI 引数: `--output docs/api/openapi.json --indent 2`。
  - [x] `--check` モード: 生成結果と commit 済み JSON を diff し非ゼロ exit (CI drift 検知用)。
- [x] `docs/api/openapi.json` を repo にコミット (確定済み設計判断)。

#### 1.10 Tests
- [x] `tests/test_api_search_meta.py` (新規):
  - [x] `test_search_response_includes_meta_envelope`: `meta.engine_version`, `meta.api_version`, `meta.schema_version` が存在し非空。
  - [x] `test_search_response_meta_includes_request_id_uuid4_hex`: `meta.request_id` が 32 桁 hex。
  - [x] `test_search_response_meta_includes_verification_url`: `meta.verification_url` が `query` `language` `dialect` を含む URL。
  - [x] `test_search_response_meta_includes_timestamp_iso8601`: `datetime.fromisoformat` で parse 可能。
  - [x] `test_search_response_meta_includes_ruleset_versions_for_language`: `meta.ruleset_versions["ancient_greek"]` が `data_versions.rules` と一致。
  - [x] `test_search_response_meta_data_versions_mirrors_top_level`: `meta.data_versions == data_versions`。
  - [x] `test_search_response_meta_includes_request_echo`: `meta.request_echo.query_form == request.query_form` 等。
  - [x] `test_search_response_meta_engine_version_matches_app_version`: `meta.engine_version == api_main._APP_VERSION`。
- [x] `tests/test_api_request_id.py` (新規):
  - [x] `test_x_request_id_header_set_when_absent`: client が送らないとき response header `X-Request-ID` が hex string。
  - [x] `test_x_request_id_header_respected_when_valid`: client が `X-Request-ID: abc123...` を送ると同値が response header に返る。
  - [x] `test_x_request_id_header_replaced_when_invalid`: 不正値 (空 / 非 hex / 異常長) は server-generated value で上書き。
  - [x] `test_request_id_present_on_error_responses`: 400/503 でも `X-Request-ID` が付く。
- [x] `tests/test_api_verification.py` (新規): `verification_url`, `request_echo`, `request_id` の再現性を検証。
- [x] `tests/test_api_main.py` の既存 `TestSearchEndpoint` で response shape を assert している箇所が壊れないことを確認。新規 `meta` の存在を明示的にテストする項目を 1 つ追加。

### Files to create / modify
- Created: `src/api/_request_context.py`
- Created: `src/api/_response_meta.py`
- Create: `src/api/_constants.py`
- Create: `scripts/export_openapi.py`
- Created: `tests/test_api_search_meta.py`
- Created: `tests/test_api_request_id.py`
- Create: `docs/api/openapi.json` (生成物、コミット)
- Modified: `src/api/_models.py` — `ResponseMeta`, `RequestEcho`, `SearchResponse.meta` 追加。
- Modified: `src/api/main.py` — request id middleware、`_build_ruleset_versions`、`search()` の response 組み立て。
...
### Post-review hardening (本コミットで追加)
- [x] `_add_request_id` の補強: ハンドラ外で出る `RuntimeError` 等も `@app.exception_handler(Exception)` 経由で 500 + `X-Request-ID` を返す。
- [x] `PROTEUS_PUBLIC_BASE_URL` の起動時検証 (`_app_lifespan` で fail-fast、誤設定で `RuntimeError`)。`validate_public_base_url` は `build_verification_url` と検証規則を共有。
- [x] `is_valid_request_id` を `re.fullmatch` ベースに変更し `"0x"` プレフィックスを拒否。テストにパラメータ追加。
- [x] CORS preflight テストを `importlib.reload` で書き直し、Starlette private API (`Middleware.kwargs`, `middleware_stack`) への依存を除去。
- [x] README "Deployment & Operations" に `PROTEUS_PUBLIC_BASE_URL` + `meta.verification_url` のプライバシー境界 (server log redact と独立) を 1 ブロック追加。`RequestEcho` docstring も同旨を明記。
- [x] `test_api_main.py` の意図的型違反箇所 (lines 299/361/689) に `# type: ignore[arg-type]` 注釈、または `tuple` への修正。
...
- [x] base URL の解決順序: env `PROTEUS_PUBLIC_BASE_URL` > `fastapi_request.base_url`。
- [x] URL query パラメータ: `q=<query_form>&language=<language>&dialect=<dialect>&max_candidates=<n>&response_language=<en|ja>`。
- [x] URL から `orthography_hint` を除外 (deprecated)。
- [x] base URL に trailing slash があってもなくても結合結果が `<base>/?q=...` の形に揃うようにする。

#### 4.2 Helper (章 1.4 と統合)
- [x] `src/api/_request_context.py` の以下を最終形で実装:
  - [x] `resolve_public_base_url(fastapi_request: Request) -> str`
  - [x] `build_verification_url(base: str, request: SearchRequest) -> str`
  - [x] `build_request_echo(request: SearchRequest) -> RequestEcho`

#### 4.3 MCP response でも同じ verification_url
- [x] 章 5 で MCP 出力に `meta.verification_url` をそのまま使い回す前提で本章 helper を再利用する。
- [x] MCP context には `fastapi_request` が無いため、env `PROTEUS_PUBLIC_BASE_URL` 未設定時の verification_url は `""` (warn ログ)。

#### 4.4 Tests
- [x] `tests/test_api_verification.py` (新規):
  - [x] `test_verification_url_uses_public_base_url_env`: monkeypatch で `PROTEUS_PUBLIC_BASE_URL=https://proteus.example/` を set すると返却 URL がそれを含む。
  - [x] `test_verification_url_uses_request_base_url_when_env_unset`: env 未設定で `http://testserver/` を含む。
  - [x] `test_verification_url_includes_query_form_urlencoded`: 非 ASCII クエリ (`λόγος`) が percent-encode される。
  - [x] `test_verification_url_includes_language_and_dialect`。
  - [x] `test_verification_url_excludes_orthography_hint_deprecated`。
  - [x] `test_verification_url_deterministic_across_calls`: 同じリクエストを 2 回送ると `verification_url` が同じ (request_id は変わる)。
  - [x] `test_request_echo_mirrors_validated_request_fields`: `request_echo.dialect_hint == "attic"` (default 適用後)。
  - [x] `test_request_echo_excludes_internal_legacy_flag`: `legacy_language_alias_used` が echo に出ない。
  - [x] `test_request_id_is_uuid4_hex_lowercase`。

### Files to create / modify
- Modified: `src/api/_request_context.py` (章 1.4 で作成済み想定)。
- Created: `tests/test_api_verification.py`.
...
#### 5.1 依存関係
- [x] `pyproject.toml` の `[project] dependencies` に `mcp>=1.0.0,<2.0.0` を追加。
- [x] `[project.optional-dependencies] dev` には MCP テスト用の追加依存は基本不要 (`mcp.shared.memory.create_connected_server_and_client_session` が標準で利用可能)。
- [x] `[project.scripts]` セクションを `pyproject.toml` に追加:
  - [x] `proteus-mcp = "mcp_server.server:main"`
- [x] `[tool.hatch.build.targets.wheel] packages` に `src/mcp_server` を追加。
- [x] mypy / pyright include パスに `src/mcp_server` が含まれることを確認 (`mypy_path = "src"` で自動的に含まれる)。

#### 5.2 パッケージ骨格
- [x] 新規ディレクトリ `src/mcp_server/`。
- [x] `src/mcp_server/__init__.py`:
  - [x] バージョン定数 `__version__` を定義 (`api.main._load_app_version` を再利用)。
  - [x] `__all__ = ["__version__"]`。
- [x] `src/mcp_server/server.py`:
  - [x] `from mcp.server.fastmcp import FastMCP` を import。
  - [x] `app = FastMCP("proteus")` を構築。
  - [x] `register_default_profiles()` を import 時に呼ぶ (API と同様)。
  - [x] tools をローカルで register。
  - [x] `def main() -> None:` で `app.run(transport="stdio")` を呼ぶ。
  - [x] ロガー `logger = logging.getLogger("proteus.mcp")` を設定。
- [x] `src/mcp_server/tools/__init__.py` (空、または `from .search import register_search_tool`)。
- [x] `src/mcp_server/tools/search.py`:
  - [x] `@app.tool("ancient_phonology.search")` で関数を定義。
  - [x] 入力 Pydantic モデル `McpSearchInput`:
    - [x] `query_form: str`
    - [x] `source_language: str = "ancient_greek"`
    - [x] `dialect_hint: str | None = None`
    - [x] `max_candidates: int = 10`
    - [x] `response_language: Literal["en", "ja"] = "en"`
  - [x] 出力 Pydantic モデル `McpSearchOutput`:
    - [x] `candidates: list[SearchHit]` — REST と同じ `SearchHit`。
    - [x] `query: str`
    - [x] `query_ipa: str`
    - [x] `query_mode: Literal["Full-form", "Short-query", "Partial-form"]`
    - [x] `truncated: bool`
    - [x] `meta: ResponseMeta`。
  - [x] handler 関数で `_run_search_for_mcp` adapter を呼ぶ。
- [x] `src/mcp_server/_search_adapter.py`:
  - [x] `_run_search_for_mcp(input: McpSearchInput) -> McpSearchOutput`。
  - [x] 内部で REST の `search()` パスと同じ手順:
    - [x] `request = SearchRequest(query_form=..., language=..., dialect_hint=..., max_candidates=..., response_language=...)` を組み立て validator を通す。
    - [x] `deps = api_main._load_search_dependencies(request.language)` を呼ぶ。
    - [x] 直接 `phonology_search.search_execution(...)` を呼ぶ (`api/main.py:586-606` と同じ kwargs)。
    - [x] hits は `_build_search_hit` (`src/api/_hit_formatting.py`) を再利用。
    - [x] `ResponseMeta` を `build_response_meta(...)` (章 1.4) で再利用。
    - [x] `verification_url` は MCP では request の base_url が無いため env `PROTEUS_PUBLIC_BASE_URL` 必須化、未設定なら `""` (log warn)。

#### 5.3 Tool schema export
- [x] `scripts/export_mcp_schema.py` (新規) — MCP tool I/O schema を JSON で `docs/mcp/tools.json` に書き出す。
  - [x] `from mcp_server.server import app` してから `app.list_tools()` の結果をシリアライズ。
  - [x] `--check` モードで commit 済み JSON との diff を返す (CI drift 検知)。

#### 5.4 Example config
- [x] `docs/mcp/example-claude-desktop.json` を作成:
  ```json
  {
    "mcpServers": {
      "proteus": {
...
- [x] `src/mcp_server/server.py` でロガー `logger = logging.getLogger("proteus.mcp")`。
- [x] tool invocation 時に request_id (UUID4) + sanitized query (`_summarize_query_for_logs` を再利用) をログ。
- [x] MCP server から `phonology_search.search_execution` を呼ぶときに raw query をログするかは REST と同じ env flag `PROTEUS_LOG_RAW_SEARCH_QUERY` で制御。

#### 5.6 Tests
- [x] `tests/test_mcp_search_tool.py` (新規):
  - [x] `test_mcp_server_lists_ancient_phonology_search_tool`: `app.list_tools()` に `ancient_phonology.search` が含まれる。
  - [x] `test_mcp_search_tool_returns_candidates`: 既知クエリ (`λόγος` など) で `candidates` が 1 件以上。
  - [x] `test_mcp_search_tool_returns_confidence_and_distance`。
  - [x] `test_mcp_search_tool_returns_applied_rules`: Koine query で `rules_applied` に CCH-009 を含む (or 同等の Phase 1 で確立した assertion)。
  - [x] `test_mcp_search_tool_returns_meta_envelope`: `meta.engine_version`, `meta.ruleset_versions`, `meta.request_id`, `meta.verification_url`。
  - [x] `test_mcp_search_tool_respects_max_candidates`: `max_candidates=2` で 2 件以内。
  - [x] `test_mcp_search_tool_validates_unsupported_language`: 未登録 language で MCP error。
  - [x] `test_mcp_search_tool_validates_dialect_hint`: 未対応 dialect で error。
  - [x] `test_mcp_search_tool_uses_default_dialect_when_omitted`。
  - [x] `test_mcp_search_tool_response_language_ja`: ja 指定で日本語 prose が返る。
  - [x] `test_mcp_search_tool_verification_url_uses_env_when_set`。
  - [x] `test_mcp_search_tool_verification_url_empty_when_env_unset`。
- [x] `tests/test_mcp_server_init.py` (新規):
  - [x] `test_mcp_server_module_importable`: `import mcp_server.server` が succeeds。
  - [x] `test_mcp_server_entrypoint_script_resolves`: `importlib.metadata.entry_points()` に `proteus-mcp` が出る (`pip install -e .` 後)。
  - [x] `test_mcp_server_version_matches_app_version`: `mcp_server.__version__ == api.main._APP_VERSION`。
- [x] MCP test fixture は `app` を直接呼ぶ in-process tests とする (stdio をモックしない)。Python MCP SDK の `create_connected_server_and_client_session` を使うのが標準。
...
- `docs/API.md` を作成。
- `docs/MCP.md` を作成。
- `docs/api/openapi.json` は既存 artifact を確認し、`docs/API.md` から参照。
- `README.md` に REST API / MCP Server セクションを追加。

### Atomic checklist

#### 6.1 `docs/API.md`
- [x] H1 タイトル `# Proteus REST API` を付け、Phase 2 v1.0 として明記。
- [x] セクション構成:
  - [x] `## Versioning` — `api_version`, `schema_version`, deprecation policy。
  - [x] `## Endpoints`:
    - [x] `POST /search` (Request / Response / errors / examples / curl)。
    - [x] `GET /languages`。
    - [x] `GET /version`。
    - [x] `GET /health`, `GET /ready`。
    - [x] `GET /` (frontend HTML), `GET /changelog`。
  - [x] `## Request / Response Models` — `SearchRequest`, `SearchResponse`, `SearchHit`, `RuleStep`, `OrthographicNote`, `DataVersions`, `ResponseMeta`, `RequestEcho`, `LanguageInfo`, `LanguagesResponse`, `VersionInfo` を semantic table で。
  - [x] `## Error Responses` — 400, 422, 503 と detail 文字列。
  - [x] `## Reproducibility` — `request_id`, `verification_url`, `request_echo` の使い方。
  - [x] `## Deprecation Policy` — `Deprecation: true` header, `Link: deprecation` header, `X-Proteus-Migration`, `orthography_hint` の deprecated 注記。
  - [x] `## Environment Variables` — `PROTEUS_ALLOWED_ORIGINS`, `PROTEUS_PUBLIC_BASE_URL`, `PROTEUS_APP_VERSION`, `PROTEUS_ENABLE_API_DOCS`, `PROTEUS_LOG_RAW_SEARCH_QUERY`, `PROTEUS_DISABLE_STARTUP_WARMUP`, `PROTEUS_GIT_SHA`, `PROTEUS_BUILD_TIMESTAMP`。
  - [x] `## OpenAPI Schema` — `docs/api/openapi.json` への参照と再生成コマンド。
  - [x] `## Examples` — curl, httpie, Python (`httpx`) でクエリする 3 例。
- [x] `docs/API.md` は英語、重要 section は日本語要約を付ける (REQUIREMENTS / ROADMAP と一貫)。

#### 6.2 `docs/MCP.md`
- [x] H1 `# Proteus MCP Server`。
- [x] セクション:
  - [x] `## Overview` — `ancient_phonology.search` ツールが提供する機能。
  - [x] `## Installation` — `uv pip install proteus` または `pipx install proteus`。
  - [x] `## Running` — `proteus-mcp` (stdio transport)。
  - [x] `## Claude Desktop config` — `docs/mcp/example-claude-desktop.json` 参照。
  - [x] `## Tool Reference: ancient_phonology.search` — input / output schema、例。
  - [x] `## Output Schema` — `McpSearchOutput` の全フィールド表。
  - [x] `## Verification & Reproducibility` — `meta.verification_url`, `meta.request_id`。
  - [x] `## Limitations` — 単一 transport (stdio)、認証なし、Phase 2 prototype と明記。
  - [x] `## Future Tools` — `ancient_phonology.explain_rule`, `ancient_phonology.list_languages`, `ancient_phonology.list_rules`, `ancient_phonology.compare_candidates` (`docs/REQUIREMENTS.md` §4.4)。
- [x] `docs/mcp/example-claude-desktop.json` を確認し MCP.md からリンク。
- [x] `docs/mcp/tools.json` (章 5.3 で生成) へのリンクを記載。

#### 6.3 OpenAPI artifact
- [x] `docs/api/openapi.json` を `scripts/export_openapi.py` (章 1.9) で生成し commit する。
- [x] `docs/API.md` の `## OpenAPI Schema` セクションで `uv run python scripts/export_openapi.py --output docs/api/openapi.json` を再生成コマンドとして文書化。
- [x] CI で `scripts/export_openapi.py --check` を実行し、生成結果と commit 済み JSON を diff し非ゼロ exit させる (確定済み設計判断)。

#### 6.4 README 更新
- [x] `README.md` の API 導線を "REST API" と "MCP Server" に更新。
- [x] `## REST API` セクションを追加: 簡単な curl 例 + `docs/API.md` へのリンク。
- [x] `## MCP Server` セクションを追加: `proteus-mcp` 起動例 + `docs/MCP.md` へのリンク。
- [x] 既存の日英併記スタイルを継承する。

#### 6.5 CODEMAPS 更新
- [x] `docs/CODEMAPS/api.md` を更新:
  - [x] 新規モデル (`ResponseMeta`, `RequestEcho`, `LanguageInfo`, `LanguagesResponse`, `VersionInfo`) を追加。
  - [x] 新規 endpoint (`GET /languages`, `GET /version`) を追加。
  - [x] middleware (`_add_request_id`) を追加。
- [x] `docs/CODEMAPS/INDEX.md` に MCP entry を追加 (`docs/CODEMAPS/mcp.md` 新規)。
- [x] `docs/CODEMAPS/mcp.md` (新規):
  - [x] `src/mcp_server/server.py`, `src/mcp_server/tools/search.py`, `src/mcp_server/_search_adapter.py`, `tests/test_mcp_search_tool.py` の関係図。

#### 6.6 CHANGELOG
- [x] `CHANGELOG.md` の `## Unreleased` / `### Phase 2: REST API and MCP Prototype` を具体化:
  - [x] Added: `/languages`, `/version`, `meta` envelope, `X-Request-ID`, `verification_url`, `proteus-mcp` script, `ancient_phonology.search` MCP tool。
  - [x] Documentation: `docs/API.md`, `docs/MCP.md`, `docs/api/openapi.json`, `docs/mcp/tools.json`。
  - [x] Compatibility: 既存 `data_versions` トップレベルは維持。

### Files to create / modify
- Created: `docs/API.md`
- Created: `docs/MCP.md`
- Confirmed and linked: `docs/mcp/example-claude-desktop.json`
- Confirmed and linked: `docs/mcp/tools.json` (生成物)
- Confirmed and linked: `docs/api/openapi.json` (生成物)
- Created: `docs/CODEMAPS/mcp.md`
- Modified: `docs/CODEMAPS/api.md`
...
- [x] `tests/test_openapi.py`:
  - [x] `test_openapi_schema_is_valid_jsonschema`: `app.openapi()` の結果が OpenAPI 3.1 として妥当 (`jsonschema` で簡易検証)。
  - [x] `test_openapi_schema_contains_search_endpoint`。
  - [x] `test_openapi_schema_contains_languages_endpoint`。
  - [x] `test_openapi_schema_contains_version_endpoint`。
  - [x] `test_openapi_artifact_matches_app_openapi`: `docs/api/openapi.json` と `app.openapi()` が一致 (drift 検知)。fail 時には修復コマンド (`uv run python scripts/export_openapi.py --output docs/api/openapi.json`) を message に含める。
- [x] `tests/test_mcp_schema.py` (任意):
  - [x] `test_mcp_tools_artifact_matches_runtime`: `docs/mcp/tools.json` が `app.list_tools()` と一致。
  - [x] `test_mcp_search_tool_input_schema_includes_required_fields`。
  - [x] `test_mcp_search_tool_output_schema_includes_meta_envelope`。

#### 7.4 CI workflow 更新
- [x] `.github/workflows/ci.yml` の `test` ジョブで `uv sync --all-extras --dev` が `mcp>=1.0.0` を含む依存をインストールすることを確認。
- [x] `mcp` の wheel が Python 3.11/3.12 両方で available か PyPI で確認。
- [x] CI に新しいステップを追加:
  - [x] `Validate OpenAPI artifact`: `uv run python scripts/export_openapi.py --check` (drift がないことを確認)。
  - [x] `Validate MCP schema artifact`: `uv run python scripts/export_mcp_schema.py --check`。

#### 7.5 Linting / typing
- [x] `src/mcp_server/` 配下を mypy / pyright が型チェックすることを確認 (`tool.mypy.files = ["src"]` で含まれる)。
- [x] `uv run mypy` / `uv run pyright` が green であること。
...
- [x] **API returns structured candidates**
  - [x] 検証: `uv run pytest tests/test_api_main.py::TestSearchEndpoint -q` が pass。
  - [x] 検証: `POST /search` の candidate shape は `tests/test_api_main.py::TestSearchEndpoint` と full suite で確認する。手動 `curl` は自動テストで代替。
- [x] **API returns applied rules and explanations**
  - [x] 検証: `SearchHit.rules_applied` / `SearchHit.explanation` は API/search tests で確認する。
  - [x] 検証: `tests/test_search.py` / `tests/test_search_annotation.py` の Phase 1 受け入れテストが緑。
- [x] **MCP server can answer a query through the search engine**
  - [x] 検証: `uv run pytest tests/test_mcp_search_tool.py::test_mcp_search_tool_returns_candidates -q`。
  - [x] 手動検証: Claude Desktop 手動確認は release 前 operator smoke とし、自動テストと `docs/mcp/example-claude-desktop.json` で Phase 2 acceptance を満たす。
- [x] **MCP response includes candidates, confidence, applied rules, and metadata**
  - [x] 検証: `tests/test_mcp_search_tool.py::test_mcp_search_tool_returns_meta_envelope` が pass。
  - [x] 検証: `tests/test_mcp_search_tool.py::test_mcp_search_tool_returns_applied_rules` が pass。
- [x] **API and MCP output schemas are documented**
  - [x] 検証: `docs/API.md` 存在し全 endpoint / 全モデルを記載。
  - [x] 検証: `docs/MCP.md` 存在し tool reference を記載。
  - [x] 検証: `docs/api/openapi.json` が `app.openapi()` と一致 (`scripts/export_openapi.py --check`)。

#### 8.2 Phase 2 完了前の最終チェック
- [x] フル test を 1 度通す: `uv run pytest -q` (Phase 2 開始時の baseline を超える pass 数になっていること)。
- [x] `uv run mypy` / `uv run pyright` 共に green。
- [x] `uv build --wheel` が成功し、`mcp_server` package が wheel 内に含まれる。
- [x] `uv run proteus-mcp` の direct smoke は stdio server のため自動 MCP tests と wheel package validation で代替。operator smoke は release 後に MCP client から実施する。
- [x] Phase 2 の release tag はコミット後に作成する: `git tag -a v0.3.0 -m "Release v0.3.0"` (additive changes のみ、minor bump)。未コミット変更にはタグを打たない。
- [x] `docs/ROADMAP.md` の Phase 2 section を `## 4. Phase 2: REST API and MCP Prototype ✅ COMPLETE` に更新し、各受け入れ基準にチェックを入れる。
- [x] `CHANGELOG.md` を `## [0.3.0] - 2026-05-18` として release date 付きで確定。
- [x] `pyproject.toml` の `version` を `0.3.0` に bump (Phase 1 までは `0.2.1`)。
...
5. **章 1.9 + 章 6.3 OpenAPI artifact** — `scripts/export_openapi.py` + `docs/api/openapi.json` を commit + CI drift 検知。完了。
6. **章 5 MCP server prototype** — 依存追加 + パッケージ作成 + `ancient_phonology.search` tool + test。完了。
7. **章 6 documentation** — `docs/API.md`, `docs/MCP.md`, README / CODEMAPS / CHANGELOG。完了。
8. **章 7 CI 統合** — openapi / mcp schema drift 検知の CI step を追加。完了。
...
- [x] `GET /version`, `GET /languages` が green-on-CI で動作する。
- [x] `POST /search` レスポンスに `meta.engine_version`, `meta.ruleset_versions`, `meta.request_id`, `meta.verification_url`, `meta.request_echo` が含まれる。
- [x] `X-Request-ID` レスポンスヘッダが全 endpoint で常に付与される。
- [x] `proteus-mcp` script がインストール可能で stdio transport で起動する。
- [x] `ancient_phonology.search` MCP tool が REST `/search` と同等の構造化レスポンス + meta envelope を返す。
- [x] `docs/API.md`, `docs/MCP.md`, `docs/api/openapi.json`, `docs/mcp/tools.json`, `docs/mcp/example-claude-desktop.json` がコミットされている。
- [x] CI が openapi drift と MCP schema drift を検出して fail させる。
- [x] Phase 2 開始時の baseline `uv run pytest -q` pass 数を、新規追加 test 分だけ上回って green。
- [x] `docs/ROADMAP.md` Phase 2 が ✅ COMPLETE。
- [x] `pyproject.toml` の version が `0.3.0`。
