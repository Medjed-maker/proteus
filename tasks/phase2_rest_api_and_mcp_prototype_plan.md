...
- [ ] `ResponseMeta` の field validator で `timestamp` を `DataVersions._validate_timestamp` と同じパターンで検証。
...
- [ ] `src/api/_constants.py` (新規) を作成:
  - [ ] `API_VERSION = "1.0"`
  - [ ] `SCHEMA_VERSION = "1.0.0"`
- [ ] `SCHEMA_VERSION` の bump policy を docstring に書く。`SearchResponse` / `LanguagesResponse` の互換性が崩れない追加は minor、追加+互換性破壊は major。
- [ ] 章 1 の `ResponseMeta.api_version` と章 3 の `VersionInfo.api_version` は同じ定数を読む。
...
#### 2.1 Registry 公開関数
- [ ] `src/phonology/profiles.py` に `list_language_profiles() -> tuple[LanguageProfile, ...]` を追加。
  - [ ] `_REGISTRY_LOCK` を取得して `_REGISTRY.values()` を snapshot し、`tuple()` で凍結。
  - [ ] 戻り値は `language_id` で昇順ソート。
  - [ ] docstring: registry の current snapshot を返す。後続の register 操作は反映されない。
- [ ] `list_language_profiles` を `__all__` に追加 (`src/phonology/profiles.py:167-173`)。

#### 2.2 Pydantic モデル: `LanguageInfo` / `LanguagesResponse`
- [ ] `src/api/_models.py` に `LanguageInfo(BaseModel)` を追加。フィールド:
  - [ ] `language_id: str`
  - [ ] `display_name: str`
  - [ ] `default_dialect: str`
  - [ ] `supported_dialects: list[str]`
  - [ ] `status: Literal["pilot", "experimental", "stable"]` — API 層で `language_id` ベースに決める。`ancient_greek` は `"pilot"`、他 default `"experimental"`。
  - [ ] `ruleset_version: str` — 該当言語の `get_rules_version` 集計 max。エラー時は `"unknown"`。
  - [ ] `lexicon_schema_version: str` — `_load_lexicon_document(language).schema_version`。エラー時は `"unknown"`。
  - [ ] `matrix_version: str` — `_load_distance_matrix_with_meta(language)[1]["version"]`。エラー時は `"unknown"`。
  - [ ] `description: str` — API 層で `language_id` ベースの固定マッピング。未知の language は `""`。
- [ ] `LanguagesResponse(BaseModel)` を追加:
  - [ ] `languages: list[LanguageInfo]`
  - [ ] `meta: ResponseMeta` — `request_echo` は `None`。
- [ ] `LanguageInfo`, `LanguagesResponse` を `__all__` に追加。

#### 2.3 `/languages` 実装
- [ ] `src/api/main.py` に `@app.get("/languages", response_model=LanguagesResponse)` を追加。
  - [ ] `phonology.profiles.list_language_profiles()` を呼ぶ。
  - [ ] 各 `LanguageProfile` に対して `LanguageInfo` を組み立てる。
  - [ ] エラー (lexicon/matrix が無い等) は当該 field を `"unknown"` にしてレスポンスを 200 で返す。
  - [ ] 全 language の build に失敗した場合のみ 503。
- [ ] HEAD `/languages` を追加 (frontend 用 lightweight probe)。

#### 2.4 言語別 LanguageInfo helper
- [ ] `_build_language_info(profile: LanguageProfile) -> LanguageInfo` を `src/api/main.py` に追加。
  - [ ] 内部で `get_rules_version(profile.rules_dir)` を try/except で実行。
  - [ ] `_load_distance_matrix_with_meta(profile.language_id)` を try/except で実行。
  - [ ] `_load_lexicon_document(profile.language_id)` を try/except で実行。
- [ ] `description` の i18n は Phase 2 では英語のみ。`response_language` をクエリで受けるかは Phase 3 以降の課題 (TODO docstring)。

#### 2.5 Tests
- [ ] `tests/test_api_languages.py` (新規):
  - [ ] `test_languages_endpoint_lists_ancient_greek`: `LanguagesResponse.languages` に `language_id == "ancient_greek"` が含まれる。
  - [ ] `test_languages_endpoint_returns_supported_dialects`: `attic` と `koine` が含まれる。
  - [ ] `test_languages_endpoint_returns_ruleset_version`: `ruleset_version` が semver 文字列。
  - [ ] `test_languages_endpoint_includes_toy_language_when_registered`: `isolated_language_registry` を使い、toy profile を register したら enumeration に含まれる。
  - [ ] `test_languages_endpoint_sorted_by_language_id`: 戻り値が `language_id` で昇順。
  - [ ] `test_languages_endpoint_includes_response_meta`: `meta.engine_version` 等が章 1 と同じく含まれる。
  - [ ] `test_languages_endpoint_returns_unknown_on_missing_assets`: monkeypatch で `get_rules_version` を raise させて `"unknown"` になる。
- [ ] `tests/test_language_profiles.py` に `list_language_profiles` の単体テストを追加:
  - [ ] `test_list_language_profiles_snapshot_returns_registered_profiles`
  - [ ] `test_list_language_profiles_sorted_by_language_id`
  - [ ] `test_list_language_profiles_returns_tuple`
...
- [ ] `src/api/_models.py` に `VersionInfo(BaseModel)` を追加。フィールド (確定済み: ミニマル):
  - [ ] `engine_version: str` — `_APP_VERSION`。
  - [ ] `api_version: str` — `API_VERSION` (`"1.0"`)。
  - [ ] `schema_version: str` — `SCHEMA_VERSION` (`"1.0.0"`)。
  - [ ] `rule_schema_version: str` — `data/schemas/phonology_rule_file.schema.json` の `$id` 文字列。schema が無ければ `""`。
  - [ ] `build_timestamp: str` — env `PROTEUS_BUILD_TIMESTAMP` (未設定なら `""`)。
  - [ ] `git_sha: str` — env `PROTEUS_GIT_SHA` (未設定なら `""`)。
  - [ ] `python_version: str` — `sys.version_info` から `"X.Y.Z"` 形式。
  - [ ] `mcp_server_version: str` — 章 5 の MCP server version。Phase 2 初期は `engine_version` と同値。
- [ ] `VersionInfo` を `__all__` に追加。

#### 3.2 Rule schema version helper
- [ ] `src/api/main.py` に `_load_rule_schema_version() -> str` を追加。
  - [ ] `data/schemas/phonology_rule_file.schema.json` を `@lru_cache` で読む。
  - [ ] `$id` 文字列 (URL) を返す。schema が無ければ `""`。
- [ ] URL のまま返す (一意性確保)。

#### 3.3 `/version` 実装
- [ ] `src/api/main.py` に `@app.get("/version", response_model=VersionInfo)` を追加。
- [ ] `HEAD /version` を追加。
- [ ] `VersionInfo` instance はリクエスト毎に組み立てる (lightweight)。

#### 3.4 Tests
- [ ] `tests/test_api_version.py` (新規):
  - [ ] `test_version_endpoint_returns_engine_version`: `engine_version == api_main._APP_VERSION`。
  - [ ] `test_version_endpoint_returns_api_version`: `api_version == API_VERSION`。
  - [ ] `test_version_endpoint_returns_schema_version`: semver pattern。
  - [ ] `test_version_endpoint_returns_rule_schema_id`: `rule_schema_version` startswith `"https://"` and ends with `.schema.json`。
  - [ ] `test_version_endpoint_python_version_format`: `re.fullmatch(r"\d+\.\d+\.\d+", value)`。
  - [ ] `test_version_endpoint_build_timestamp_env_override`: `monkeypatch.setenv("PROTEUS_BUILD_TIMESTAMP", "2026-05-15T00:00:00Z")` で値が反映。
  - [ ] `test_version_endpoint_git_sha_env_override`: `monkeypatch.setenv("PROTEUS_GIT_SHA", "abcdef12")` で値が反映。
  - [ ] `test_version_endpoint_head_returns_204`。
...
3. **章 2 `/languages`** — `list_language_profiles()` 追加。新言語追加への基盤。
