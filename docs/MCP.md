...
search engine and Buck reference data to MCP clients over stdio transport. It
currently registers:
...
The Buck tools expose provisional, normalized reference metadata through the
internal Buck service. They do not depend on REST endpoints, do not alter search
ranking, and do not return Buck source text or long quotations.
...
## Tool Reference: Buck Reference Tools

The Buck tools are read-only accessors for normalized Ancient Greek Buck
reference data. They currently support only `source_language="ancient_greek"`.
All Buck responses include top-level `metadata` with `status`, `review_status`,
`citation_ready`, and `review_note`; each returned rule, dialect, or glossary
item repeats `status`, `review_status`, and `citation_ready`.

When `citation_ready=false`, clients must treat the result as provisional,
not expert-reviewed discovery metadata. Do not present it as citation-ready
evidence, and do not expand references into Buck source text or long quotations.
...
| Field | Type | Default | Notes |
| --- | --- | --- | --- |
| `rule_id` | string or null | `null` | Exact Buck rule id. |
| `category` | string or null | `null` | Exact normalized category. |
| `dialect` | string or null | `null` | Matches rule `affected_dialects`. |
| `section` | string, integer, number, or null | `null` | Canonicalized by the Buck service. |
| `source_language` | string | `ancient_greek` | Only Ancient Greek is supported. |
| `max_results` | integer | `50` | Range `1` to `200`. |

Output fields:

| Field | Type | Notes |
| --- | --- | --- |
| `rules` | array of `McpBuckRuleInfo` | Matching normalized Buck rule summaries. |
| `count` | integer | Number of returned rules. |
| `metadata` | `McpBuckMetadata` | Review and citation boundary metadata. |
...
| Field | Type | Default | Notes |
| --- | --- | --- | --- |
| `dialect_id` | string | required | Exact Buck dialect id. |
| `include_rules` | boolean | `true` | Include linked rule details. |
| `include_inherited` | boolean | `true` | Walk parent dialect/group rules. |
| `source_language` | string | `ancient_greek` | Only Ancient Greek is supported. |
...
| Field | Type | Notes |
| --- | --- | --- |
| `dialect` | `McpBuckDialectInfo` | Matching dialect catalog entry. |
| `rules` | array of `McpBuckRuleInfo` | Linked rule summaries, if requested. |
| `metadata` | `McpBuckMetadata` | Review and citation boundary metadata. |
...
| Field | Type | Default | Notes |
| --- | --- | --- | --- |
| `word` | string or null | `null` | Exact word after service-level NFC normalization. |
| `standard_form` | string or null | `null` | Exact standard form after service-level NFC normalization. |
...
| Field | Type | Notes |
| --- | --- | --- |
| `entries` | array of `McpBuckGlossaryEntryInfo` | Matching normalized glossary examples. |
